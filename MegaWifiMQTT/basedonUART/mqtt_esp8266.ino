...
const char* ssid = "Syergie";
const char* password = "1234567890";

IPAddress mqtt_server(167, 205, 76, 205);
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if(strcmp(topic,"2spc_speed") == 0){
    client.publish("confirm_SPC2","Received");
    Speed = value;
    sprintf(buf_Tx,"\t%dS\t\n",Speed);
    mySerial.print(buf_Tx);
  } 
  else if(strcmp(topic,"2spc_steer") == 0){
    client.publish("confirm_SPC2","Received");
    steer = value;
    sprintf(buf_Tx,"\t%dS\t\n",steer);
    mySerial.print(buf_Tx);
    ...
    client.publish("2PumpSpeed",dtostrf(rpm_2, 4, 0, buf));
...
void reconnect() {
  ...
  while (!client.connected()) {
    ...
    if (client.connect("ESP8266Client2")) {
      ...
      // ... or not...
      // ... and resubscribe
      client.subscribe("2spc_speed");
      client.subscribe("2spc_steer");
